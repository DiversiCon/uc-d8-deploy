...
class Showcase {
   ...
   * this function, and related classes, is such that components can add their
   ...
   *   Classes array.
   */
  public static function buildBodyClasses($class = NULL) {
  ...
  public function getIndexContent($class, $subclass) {
    ...
    $content = [];
    foreach ($this->getIndex($class, $subclass) as $id) {
      if (isset($this->itemDefinitions[$id]) && $this->itemDefinitions[$id]) {
        if (!$this->itemDefinitions[$id]->isHiddenOnIndex() && $this->itemDefinitions[$id]->isEnabled()) {
          $content[$id] = [
            ...
            'categories' => $this->itemDefinitions[$id]->getCategories(),
          ];
        }
      }
    }
  ...
  public function getStatus() {
    ...
    foreach ($this->itemDefinitions as $id => $item) {
      ...
      // Validate this item.
      $status[$id] = $item->validate($theme_paths);
  ...
  private function getThemePaths() {
    ...
    // Search through all installed themes.
    // Skip duplicates and core/contributed themes.
    foreach ($this->themeHandler->listInfo() as $theme) {
      $path = $theme->getPath();
      if (!in_array($path, $theme_paths)) {
        if (substr($path, 0, 5) == 'core/') {
          if (substr($path, 0, 15) == 'themes/contrib/') {
            $theme_paths[] = $theme->getPath();
          }
        }
      }
    }
  ...
  protected function loadItems($include_disabled = FALSE) {
    ...
    if (!$this->itemDefinitions) {
      ...
      foreach ($this->itemDefinitions as $id => $item) {
        ...
        // for the path to simplify the URL and because the type name-spacing
    ...
    if ($component) {
      ...
      if ($items) {
        ...
        if ($component->getBodyClass()) {
          $this::buildBodyClasses($component->getBodyClass());
        }
      }
  ...
  public function renderPage($page_id) {
    ...
    if ($page) {
      ...
      if ($page->getbodyClass()) {
        $this::buildBodyClasses($page->getbodyClass());
      }
    }
