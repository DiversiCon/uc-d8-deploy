 ...
 *
 * @MigrateSource(
 *   id = "d7_node_story_image_inset",
 *   source_module = "node"
 * )
 */
class D7NodeStoryImageInset extends DrupalSqlBase {
  ...
  public function prepareRow(Row $row) {
    ...
    // Map of files to include.
    $helper = new MigrateHelper();
    $include = $helper->getInsetImages();

    // Skip anything not in the include map.
    $key = $row->getSourceProperty('entity_id') . ':' . $row->getSourceProperty('field_maincontentimage_fid');
