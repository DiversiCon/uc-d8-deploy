...
/**
 * Provides a resource to get lab images.
 *
 * @RestResource(
 *   id = "lab_images_rest_resource",
 *   label = @Translation("Lab images"),
 *   uri_paths = {
 *     "canonical" = "/migrate/lab_images"
 *   }
 * )
 */
class LabImagesRestResource extends ResourceBase {
          ...
          foreach ($media_fields as $media_field) {
            ...
            if (isset($source_row[$media_field]) && !empty($source_row[$media_field])) {
              ...
              foreach ($source_row[$media_field] as $delta => $field_values) {
                ...
                $image_metadata = $helper->normalizeImageMetadata('lab', $media_field, $field_values);
