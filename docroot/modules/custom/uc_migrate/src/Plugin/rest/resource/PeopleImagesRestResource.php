...
/**
 * Provides a resource to get people images.
 *
 * @RestResource(
 *   id = "people_images_rest_resource",
 *   label = @Translation("People images"),
 *   uri_paths = {
 *     "canonical" = "/migrate/people_images"
 *   }
 * )
 */
class PeopleImagesRestResource extends ResourceBase {
          ...
          foreach ($media_fields as $media_field) {
            ...
            if (isset($source_row[$media_field]) && !empty($source_row[$media_field])) {
              ...
              foreach ($source_row[$media_field] as $delta => $field_values) {
                ...
                $image_metadata = $helper->normalizeImageMetadata('people', $media_field, $field_values);
